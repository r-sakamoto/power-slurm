 ...
 * NOTE: free the response using xfree
 */
extern int slurm_get_node_power(char *host, uint16_t *socket_cnt, power_current_data_t **get_info)
{
	int rc;
	slurm_msg_t req_msg;
	slurm_msg_t resp_msg;
 ...
 * IN  host  - name of node to query, NULL if localhost
 * OUT energy - pointer of cache_ref_t structures 
 * 		on success or NULL other wise
 * RET 0 on success or a slurm error code
 * NOTE: free the response using xfree
 */
extern int slurm_get_cache(char *host, uint16_t *cache_socket_cnt, cache_ref_t **get_info) //cache_ref_t Define at line 1384 of slurm.h
{
	int rc;
	slurm_msg_t req_msg;
	slurm_msg_t resp_msg;
	//power_knob_get_info_req_msg_t req;
	power_knob_get_cache_info_req_msg_t req; //Define at line 2233 of slurm.h
	uint32_t cluster_flags = slurmdb_setup_cluster_flags();
	char *this_addr;
	...
	req_msg.msg_type = REQUEST_POWER_KNOB_GET_CACHE_INFO; //Define at line 4896 of src/common/slurm_protocol_defs.c
	req_msg.data     = &req;
	...
	if (resp_msg.auth_cred)
		g_slurm_auth_destroy(resp_msg.auth_cred);
	switch (resp_msg.msg_type) {
	case RESPONSE_POWER_KNOB_GET_CACHE_INFO: //line 4898 of slurm_protocol_defs.c and line 3181 req.c
		*cache_socket_cnt = ((power_knob_get_cache_info_node_resp_msg_t *) 
			resp_msg.data)->cache_socket_cnt;
		*get_info = ((power_knob_get_cache_info_node_resp_msg_t *) 
			resp_msg.data)->perf_info;
		((power_knob_get_cache_info_node_resp_msg_t *) resp_msg.data)->perf_info = NULL;
		slurm_free_power_knob_get_cache_info_node_resp_msg(resp_msg.data); //at line 1019 of slurm_protocol_defs.c 
		break;
	case RESPONSE_SLURM_RC:
	        rc = ((return_code_msg_t *) resp_msg.data)->return_code;
		slurm_free_return_code_msg(resp_msg.data);
