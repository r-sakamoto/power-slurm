/*****************************************************************************\
 *  power_allocator_none.c - power alloctor plugin for none
 *****************************************************************************
...
const char		plugin_type[]	= "power_allocator/none";
...
static void *_get_allocator_none_loop(void);

static void stop_get_allocator_none_loop(void);
...
int init( void )
{
	...
	verbose( "power_allocator: Power allocator plugin loaded" );

	//pthread_mutex_lock( &thread_flag_mutex );
	//if ( powerallocator_thread ) {
	//	debug2( "power allocator thread already running, not starting "
	//		"another" );
	...
	//pthread_mutex_unlock( &thread_flag_mutex );
	if (pthread_create( &powerallocator_thread, &attr, _get_allocator_none_loop, NULL))
