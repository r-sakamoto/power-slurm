/*****************************************************************************\
 *  select_linear_power.c - node selection plugin for simple one-dimensional
...
#include "src/plugins/select/linear_power/select_linear_power.h"
 ...
 */
const char plugin_name[]       	= "Linear node selection power plugin";
const char plugin_type[]       	= "select/linear_power";
...
static uint16_t have_power_limit;
static uint32_t power_budget;
static uint32_t core_num_sock;
static uint32_t total_node_num;
static uint32_t idle_node_cpu_power;
static uint32_t idle_node_mem_power;
...
#define MAXITEM 20
static int ssplit( char *str, const char *delim, char *outlist[] ) {
    char    *tk;
    int     cnt = 0;
    tk = strtok( str, delim );
...
static uint32_t pparse_rapl_package(char *comment){
    ...
    buff = xmalloc(sizeof(char) * strlen(comment));
    if(buff==NULL){
      return 0;
    }

    strncpy(buff, comment,strlen(comment));

    cnt = ssplit( buff, ", :" , shoppinglst );
    if(cnt<0){
      xfree(buff);
      return 0;  
    }
    ...
    for(i=0;i<cnt;i++){
        n = ssplit(shoppinglst[i], "=", data);
        if(n == 2){
          if(strcmp("PACKAGE", data[0])==0){
            power = atoi(data[1]);
          }
        }
    }
...
static uint32_t pparse_rapl_dram(char *comment){
    char    *shoppinglst[MAXITEM];
    char    *data[MAXITEM];
    int     i, cnt;
    int n;
...
static int _job_power_test(struct cr_record *cr_ptr,
			     struct job_record *job_ptr)
{
	...
	int use_package;
	int tmp_use_package;
	uint32_t tmp_total_power = 0;
	struct job_record *tmp_job;
	int core_num_per_socket = core_num_sock;
	int idle_node_power = idle_node_cpu_power + idle_node_mem_power;
	int tot_idle_node_power;

	xassert(cr_ptr);
	xassert(cr_ptr->nodes);
 
	use_package = job_ptr->cpu_cnt/core_num_per_socket;
	tot_idle_node_power = total_node_num * idle_node_power;

	debug3("############################################");
	debug3(" run job len  %u",cr_ptr->run_job_len);
	debug3(" tot job len  %u",cr_ptr->tot_job_len);
	debug3("--------------------------------------------");
	debug3(" this job id %u", job_ptr->job_id);
	debug3("   cpu_cnt %u", job_ptr->cpu_cnt);
	debug3("   use package %u", job_ptr->cpu_cnt/core_num_per_socket);
	debug3("   use node    %u", (job_ptr->cpu_cnt/core_num_per_socket)/2);
	debug3("   package %u",pparse_rapl_package(job_ptr->comment));
	debug3("   dram %u",pparse_rapl_dram(job_ptr->comment));
	debug3("   job total w %u", (job_ptr->cpu_cnt/core_num_per_socket) * 
                     (pparse_rapl_package(job_ptr->comment) + pparse_rapl_dram(job_ptr->comment)));
	debug3("   --------------------");

	for(i = 0; i<cr_ptr->run_job_len; i++){
		tmp_job = find_job_record(cr_ptr->run_job_ids[i]);
		if(tmp_job){
			debug3(" preb job id %u", tmp_job->job_id);
			if(tmp_job->comment){
				debug3(" comment: %s", tmp_job->comment);
				debug3("     package %u", pparse_rapl_package(tmp_job->comment));
				debug3("     use cpu_cnt %u", tmp_job->cpu_cnt);
				debug3("     use package %u", tmp_job->cpu_cnt/core_num_per_socket);
				debug3("     use node    %u",  (tmp_job->cpu_cnt/core_num_per_socket)/2 );
				debug3("     dram    %u", pparse_rapl_dram(tmp_job->comment));
				debug3("     job_total w %u",  tmp_job->cpu_cnt/core_num_per_socket 
                                               * (pparse_rapl_package(tmp_job->comment) + pparse_rapl_dram(tmp_job->comment)));
				tmp_total_power += (tmp_job->cpu_cnt/core_num_per_socket)*pparse_rapl_package(tmp_job->comment);
				tot_idle_node_power = tot_idle_node_power - ( idle_node_power * ( (tmp_job->cpu_cnt/core_num_per_socket)/2 ));
				tmp_total_power += (tmp_job->cpu_cnt/core_num_per_socket)*pparse_rapl_dram(tmp_job->comment);
				debug3("     idle node power %u",tot_idle_node_power);
			}
		}
	}

	debug3(" rev total power %u", tmp_total_power);
	debug3(" idle node power %u", tot_idle_node_power);
	if(job_ptr->comment){
		tmp_total_power += (job_ptr->cpu_cnt/core_num_per_socket) * 
                                   (pparse_rapl_package(job_ptr->comment) + pparse_rapl_dram(job_ptr->comment));
	}
 
	tmp_total_power = tmp_total_power + tot_idle_node_power - ( (job_ptr->cpu_cnt/core_num_per_socket)/2 * idle_node_power);
	debug3(" all total power %u", tmp_total_power);
	if(power_budget > tmp_total_power){
		debug3(" power const OK");
		rc =  SLURM_SUCCESS;
	}else{
		debug3(" power const NG");
	}
		      ...
		      uint32_t req_nodes, int max_share)
{
	...
	//if(rc == SLURM_SUCCESS){
	//	if(have_power_limit){
	//		rc = _job_power_test(cr_ptr, job_ptr);
	//	}
	//}
...
top:	if ((rc != SLURM_SUCCESS) && preemptee_candidates &&
		...
		while ((tmp_job_ptr = (struct job_record *)
			...
			if(rc == SLURM_SUCCESS){
				if(have_power_limit){
					rc = _job_power_test(exp_cr, job_ptr);
				}
			}  
			  ...
			  List *preemptee_job_list)
{
	...
	if(job_ptr->start_time){
		debug3("       %u   will start at %u",job_ptr->job_id, job_ptr->start_time);
		will_time = localtime(&(job_ptr->start_time));
		debug3("(%d-",will_time->tm_year+1900);
		debug3("%d-",will_time->tm_mon+1);
		debug3("%d ",will_time->tm_mday);
		debug3("%d:",will_time->tm_hour);
		debug3("%d:",will_time->tm_min);
		debug3("%d)\n",will_time->tm_sec);
	}
	...
	_free_cr(exp_cr);
	FREE_NULL_BITMAP(orig_map);
	debug3("   _will_run_test return val : %u", rc);
	return rc;
}
...
uint32_t _powercap_get_system_cap(void)
{
        ...
        if ((tmp_ptr = strstr(sched_params, "system_cap_watts=INFINITE"))) {
                cap_watts = INFINITE;
        } else if ((tmp_ptr = strstr(sched_params, "system_cap_watts=UNLIMITED"))) {
                cap_watts = INFINITE;
        } else if ((tmp_ptr = strstr(sched_params, "system_cap_watts="))) {
                cap_watts = strtol(tmp_ptr + 17, &end_ptr, 10);
...
extern int init ( void )
{
	...
	char *select_params, *tmp_ptr;

	select_params = slurm_get_select_params();
	if(select_params)
		debug3("## select_params string:%s",select_params);
	else
		debug3("## select_params NULL");
	cr_type = slurmctld_conf.select_type_param;
	if (cr_type)
		verbose("%s loaded with argument %u", plugin_name, cr_type);

 	have_power_limit = true;
	if(select_params && (tmp_ptr=strstr(select_params, "total_power_limit="))){
		power_budget = atoi(tmp_ptr + 18);
	}
	if(power_budget < 1){
		error("Invalid SelectParameters total_power_limit: %d", power_budget);
		have_power_limit = false;
	}
	
	if(have_power_limit == true){
		if(select_params && (tmp_ptr=strstr(select_params, "core_num_sock=")))
			core_num_sock = atoi(tmp_ptr + 14);
		if(core_num_sock < 1){
			error("Invalid SelectParameters core_num_sock: %d", core_num_sock);
			have_power_limit = false;
		}	
	}

	if(have_power_limit == true){
		if(select_params && (tmp_ptr=strstr(select_params, "total_node_num=")))
			total_node_num = atoi(tmp_ptr + 15);
		if(total_node_num < 1){
			error("Invalid SelectParameters total_node_num: %d", total_node_num);
			have_power_limit = false;
		}	
	}

	if(have_power_limit == true){
		if(select_params && (tmp_ptr=strstr(select_params, "idle_node_cpu_power=")))
			idle_node_cpu_power = atoi(tmp_ptr + 20);
		if(idle_node_cpu_power < 1){
			error("Invalid SelectParameters idle_node_cpu_power: %d", idle_node_cpu_power);
			have_power_limit = false;
		}	
	}

	if(have_power_limit == true){
		if(select_params && (tmp_ptr=strstr(select_params, "idle_node_mem_power=")))
			idle_node_mem_power = atoi(tmp_ptr + 20);
		if(idle_node_mem_power < 0){
			error("Invalid SelectParameters idle_node_mem_power: %d", idle_node_mem_power);
			have_power_limit = false;
		}	
	}
	xfree(select_params);
	...
	if(have_power_limit == true)
		debug3("## select_power ENABLE ");
	else
		debug3("## select_power DISABLE ");
	debug3("## TOTAL_POWER_LIMIT   :%d", power_budget);
	debug3("## TOTQAL NODE NUM     :%d", total_node_num);
	debug3("## CORE PER SOCK       :%d", core_num_sock);
	debug3("## IDLE NODE_CPU_POWER :%d", idle_node_cpu_power);
	debug3("## IDLE NODE_MEM_POWER :%d", idle_node_mem_power);		
	debug3("#############################################################");
