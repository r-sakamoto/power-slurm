...
extern void slurm_free_power_knob_get_info_req_msg(
	power_knob_get_info_req_msg_t *msg)
{
    debug3("slurm_protocol_defs.c: slurm_free_power_knob_get_info_req_msg()");
	if (msg) {
		xfree(msg);
        debug3("slurm_protocol_defs.c: slurm_free_power_knob_get_info_req_msg(): OK");
	}
}

extern void slurm_free_power_knob_get_info_node_resp_msg(
	power_knob_get_info_node_resp_msg_t *msg)
{
	if (msg) {
		xfree(msg->node_name);
		power_knob_current_destroy(msg->power_info);
		xfree(msg);
	}
}

extern void slurm_free_power_knob_get_cache_info_req_msg(
...
extern void slurm_free_power_knob_get_cache_info_node_resp_msg(
...
extern void slurm_free_power_knob_set_req_msg(
	power_knob_set_req_msg_t *msg)
{
	if (msg) {
		xfree(msg->cap_info);
		xfree(msg);
...
extern void slurm_free_power_knob_set_node_resp_msg(
	power_knob_set_node_resp_msg_t *msg)
{
	if (msg) {
		xfree(msg->node_name);
		xfree(msg->power_info);
		xfree(msg);
	}
}


extern void slurm_free_power_knob_cap_req_msg(
	power_knob_cap_req_msg_t *msg)
{
	if (msg) {
		xfree(msg);
	}
...
extern void slurm_free_power_knob_cap_resp_msg(
	power_knob_cap_resp_msg_t *msg)
{
	if (msg) {
		xfree(msg);
	}
...
extern void slurm_free_power_schedule_slurmd_req_msg(
	power_schedule_slurmd_req_msg_t *msg)
{
	if (msg) {
		xfree(msg);
	}
...
extern void slurm_free_power_schedule_slurmd_resp_msg(
	power_schedule_slurmd_resp_msg_t *msg)
{
	if (msg) {
		xfree(msg);
	}
...
{
    debug3("slurm_protocol_defs.c: slurm_free_msg_data(slurm_msg_type_t type, void *data), type='%i'", type);
...
/*
	...
	case REQUEST_POWER_KNOB_GET_CACHE_INFO:
		slurm_free_power_knob_get_cache_info_req_msg(data);
		break;
	case REQUEST_POWER_KNOB_SET:
		slurm_free_power_knob_set_req_msg(data);
		break;
	case RESPONSE_POWER_KNOB_GET_INFO:
		slurm_free_power_knob_get_info_node_resp_msg(data);
		break;
	case RESPONSE_POWER_KNOB_SET:
		slurm_free_power_knob_set_node_resp_msg(data);
		break;
...
rpc_num2string(uint16_t opcode)
{
	...
	case REQUEST_POWER_KNOB_GET_INFO:
		return "REQUEST_POWER_KNOB_GET_INFO";
	case RESPONSE_POWER_KNOB_GET_INFO:
		return "RESPONSE_POWER_KNOB_GET_INFO";
	case REQUEST_POWER_KNOB_GET_CACHE_INFO:
		return "REQUEST_POWER_KNOB_GET_CACHE_INFO";
	case RESPONSE_POWER_KNOB_GET_CACHE_INFO:
		return "RESPONSE_POWER_KNOB_GET_CACHE_INFO";
	case REQUEST_POWER_KNOB_SET:
		return "REQUEST_POWER_KNOB_SET";
	case RESPONSE_POWER_KNOB_SET:
		return "RESPONSE_POWER_KNOB_SET";
	case REQUEST_POWER_CAP_SET:
		return "REQUEST_POWER_CAP_SET";
	case RESPONSE_POWER_CAP_SET:
		return "RESPONSE_POWER_CAP_SET";

	case REQUEST_POWER_SCHEDULE_SLURMD:
		return "REQUEST_POWER_SCHEDULE_SLURMD";
		
	case RESPONSE_POWER_SCHEDULE_SLURMD:
		return "RESPONSE_POWER_SCHEDULE_SLURMD";		
	default:
