...
static void _pack_power_knob_get_info_req_msg(
			power_knob_get_info_req_msg_t *msg,
			Buf buffer,
			uint16_t protocol_version);
static int _unpack_power_knob_get_info_req_msg(
			power_knob_get_info_req_msg_t **msg,
			Buf buffer,
			uint16_t protocol_version);
static void _pack_power_knob_get_info_node_resp_msg(
			power_knob_get_info_node_resp_msg_t *msg,
			Buf buffer,
			uint16_t protocol_version);
static int _unpack_power_knob_get_info_node_resp_msg(
			power_knob_get_info_node_resp_msg_t **msg,
			Buf buffer,
			uint16_t protocol_version);
static void _pack_power_knob_get_cache_info_req_msg(
			power_knob_get_cache_info_req_msg_t *msg,
			Buf buffer,
			uint16_t protocol_version);
static int _unpack_power_knob_get_cache_info_req_msg(
			power_knob_get_cache_info_req_msg_t **msg,
			Buf buffer,
			uint16_t protocol_version);
static void _pack_power_knob_get_cache_info_node_resp_msg(
			power_knob_get_cache_info_node_resp_msg_t *msg,
			Buf buffer,
			uint16_t protocol_version);
static int _unpack_power_knob_get_cache_info_node_resp_msg(
			power_knob_get_cache_info_node_resp_msg_t **msg,
			Buf buffer,
			uint16_t protocol_version);
static void _pack_power_knob_set_req_msg(
			power_knob_set_req_msg_t *msg, Buf buffer,
			uint16_t protocol_version);
static int _unpack_power_knob_set_req_msg(
			power_knob_set_req_msg_t **msg, Buf buffer,
			uint16_t protocol_version);
static void _pack_power_knob_cap_req_msg(
			power_knob_cap_req_msg_t *msg, Buf buffer,
			uint16_t protocol_version);
static int _unpack_power_knob_cap_req_msg(
			power_knob_cap_req_msg_t **msg, Buf buffer,
			uint16_t protocol_version);
static void _pack_power_knob_cap_resp_msg(
			power_knob_cap_resp_msg_t *msg, Buf buffer,
			uint16_t protocol_version);
static int _unpack_power_knob_cap_resp_msg(
			power_knob_cap_resp_msg_t **msg, Buf buffer,
			uint16_t protocol_version);			

static void _pack_power_schedule_slurmd_req_msg(
			power_schedule_slurmd_req_msg_t *msg, Buf buffer,
			uint16_t protocol_version);
static int _unpack_power_schedule_slurmd_req_msg(
			power_schedule_slurmd_req_msg_t **msg, Buf buffer,
			uint16_t protocol_version);
static void _pack_power_schedule_slurmd_resp_msg(
			power_schedule_slurmd_resp_msg_t *msg, Buf buffer,
			uint16_t protocol_version);
static int _unpack_power_schedule_slurmd_resp_msg(
			power_schedule_slurmd_resp_msg_t **msg, Buf buffer,
			uint16_t protocol_version);		
...
pack_msg(slurm_msg_t const *msg, Buf buffer)
{
	...
	case REQUEST_POWER_KNOB_GET_INFO:
		_pack_power_knob_get_info_req_msg((power_knob_get_info_req_msg_t *)
						  msg->data, buffer,
						  msg->protocol_version);
		break;
	case RESPONSE_POWER_KNOB_GET_INFO:
		_pack_power_knob_get_info_node_resp_msg((power_knob_get_info_node_resp_msg_t *)
						  msg->data, buffer,
						  msg->protocol_version);
		break;
	case REQUEST_POWER_KNOB_GET_CACHE_INFO:
		_pack_power_knob_get_cache_info_req_msg((power_knob_get_cache_info_req_msg_t *)
						  msg->data, buffer,
						  msg->protocol_version);
		break;
	case RESPONSE_POWER_KNOB_GET_CACHE_INFO:
		_pack_power_knob_get_cache_info_node_resp_msg((power_knob_get_cache_info_node_resp_msg_t *)
						  msg->data, buffer,
						  msg->protocol_version);
		break;
	case REQUEST_POWER_KNOB_SET:
		_pack_power_knob_set_req_msg((power_knob_set_req_msg_t *)
						  msg->data, buffer,
						  msg->protocol_version);
		break;
	case RESPONSE_POWER_KNOB_SET:
		break;	
	case REQUEST_POWER_CAP_SET:
		_pack_power_knob_cap_req_msg((power_knob_cap_req_msg_t *)
						  msg->data, buffer,
						  msg->protocol_version);
		break;				  
	case RESPONSE_POWER_CAP_SET:
		_pack_power_knob_cap_resp_msg((power_knob_cap_resp_msg_t *)
						  msg->data, buffer,
						  msg->protocol_version);
		break;	

	case REQUEST_POWER_SCHEDULE_SLURMD:
		_pack_power_schedule_slurmd_req_msg((power_schedule_slurmd_req_msg_t *)
						  msg->data, buffer,
						  msg->protocol_version);
		break;
		
	case RESPONSE_POWER_SCHEDULE_SLURMD:
		_pack_power_schedule_slurmd_resp_msg((power_schedule_slurmd_resp_msg_t *)
						  msg->data, buffer,
						  msg->protocol_version);
		break;			
...
unpack_msg(slurm_msg_t * msg, Buf buffer)
{
	...
	debug("RPC of type %s(%u) received",
	rpc_num2string(msg->msg_type), msg->msg_type);

	debug("RPC of type %u received is ", msg->msg_type);
	
	debug("RPC of type %s received", rpc_num2string(msg->msg_type));
	...
	case REQUEST_EVENT_LOG:
		...
		break;
	case REQUEST_POWER_KNOB_GET_INFO:
		rc = _unpack_power_knob_get_info_req_msg((power_knob_get_info_req_msg_t **)
						  &(msg->data), buffer,
						  msg->protocol_version);
		break;
	case RESPONSE_POWER_KNOB_GET_INFO:
		rc = _unpack_power_knob_get_info_node_resp_msg(
						  (power_knob_get_info_node_resp_msg_t **)
						  &(msg->data), buffer,
						  msg->protocol_version);
		break;
	case REQUEST_POWER_KNOB_GET_CACHE_INFO:
		rc = _unpack_power_knob_get_cache_info_req_msg(
						  (power_knob_get_cache_info_req_msg_t **)
						  &(msg->data), buffer,
						  msg->protocol_version);
		break;
	case RESPONSE_POWER_KNOB_GET_CACHE_INFO:
		rc = _unpack_power_knob_get_cache_info_node_resp_msg(
						  (power_knob_get_cache_info_node_resp_msg_t **)
						  &(msg->data), buffer,
						  msg->protocol_version);
		break;
	case REQUEST_POWER_KNOB_SET:
		debug ("CASE REQUEST_POWER_KNOB_SET");
		rc = _unpack_power_knob_set_req_msg(
	...
	case REQUEST_POWER_CAP_SET:
		rc = _unpack_power_knob_cap_req_msg((power_knob_cap_req_msg_t **)
						  &(msg->data), buffer,
						  msg->protocol_version);
		break;
	case RESPONSE_POWER_CAP_SET:
		rc = _unpack_power_knob_cap_resp_msg(
						  (power_knob_cap_resp_msg_t **)
						  &(msg->data), buffer,
						  msg->protocol_version);
		break;

	case REQUEST_POWER_SCHEDULE_SLURMD:
		debug ("CASE REQUEST_POWER_SCHEDULE_SLURMD");
		rc = _unpack_power_schedule_slurmd_req_msg(
						  (power_schedule_slurmd_req_msg_t **)
						  &(msg->data), buffer,
						  msg->protocol_version);
		break;

	case RESPONSE_POWER_SCHEDULE_SLURMD:
		rc = _unpack_power_schedule_slurmd_resp_msg(
						  (power_schedule_slurmd_resp_msg_t **)
						  &(msg->data), buffer,
						  msg->protocol_version);
		break;
			  ...
			  uint16_t protocol_version)
{
	...
	if (protocol_version >= SLURM_17_02_PROTOCOL_VERSION) {
		...
		if (power_mgmt_data_unpack(&node->power, buffer,
			...
			goto unpack_error;
		if (power_knob_data_unpack((power_data_t **)&(node->power_info), buffer, 1)
		    != SLURM_SUCCESS)
			goto unpack_error;
...
//		pack32(build_ptr->power_allocatorinterval, buffer);
...
//		safe_unpack32(&build_ptr->power_allocatorinterval, buffer);
...
static void 
_pack_power_knob_get_info_req_msg(power_knob_get_info_req_msg_t *msg,
			Buf buffer,
			uint16_t protocol_version)
{
	xassert(msg != NULL);
	pack32(msg->dummy,buffer);	
}
static int 
_unpack_power_knob_get_info_req_msg(power_knob_get_info_req_msg_t **msg,
			Buf buffer,
			uint16_t protocol_version)
{
	power_knob_get_info_req_msg_t *msg_ptr;

	xassert(msg!=NULL);

	msg_ptr = xmalloc(sizeof(power_knob_get_info_req_msg_t));
	*msg = msg_ptr;

	safe_unpack32(&msg_ptr->dummy, buffer);

	return SLURM_SUCCESS;

unpack_error:
	slurm_free_power_knob_get_info_req_msg(msg_ptr);
	*msg=NULL;
	return SLURM_ERROR;
...
static void 
_pack_power_knob_set_req_msg(power_knob_set_req_msg_t *msg, Buf buffer,
				uint16_t protocol_version)
{
	int i;
	xassert(msg != NULL);

	pack16(msg->socket_cnt,buffer);
	for(i = 0; i < msg->socket_cnt; i++){
		power_knob_cap_pack(&msg->cap_info[i],buffer);
	}
}
static int 
_unpack_power_knob_set_req_msg(power_knob_set_req_msg_t **msg, Buf buffer,
				uint16_t protocol_version)
{
	int i;
	power_knob_set_req_msg_t *msg_ptr;
	power_capping_data_t *cap_info;
	uint16_t socket_cnt;
	debug ("inside unpack of slurm_protocol_pack");
	xassert(msg!=NULL);

	msg_ptr = xmalloc(sizeof(power_knob_set_req_msg_t));
	*msg = msg_ptr;

	safe_unpack16(&msg_ptr->socket_cnt,buffer);
		
	if(msg_ptr->socket_cnt < 1){
		goto unpack_error;
	}

	cap_info = xmalloc(sizeof( power_capping_data_t) * msg_ptr->socket_cnt);
	memset(cap_info,0,sizeof( power_capping_data_t) * msg_ptr->socket_cnt);
	msg_ptr->cap_info = cap_info;

	for(i=0; i < msg_ptr->socket_cnt; i++){
		power_knob_cap_unpack(&cap_info[i], buffer, 0);
	}

	return SLURM_SUCCESS;

unpack_error:
	slurm_free_power_knob_set_req_msg(msg_ptr);
	*msg=NULL;
	return SLURM_ERROR;

}
static void
_pack_power_knob_get_info_node_resp_msg(power_knob_get_info_node_resp_msg_t *msg,
				Buf buffer, uint16_t protocol_version)
{
	unsigned int i;
	...
	if (protocol_version >= SLURM_15_08_PROTOCOL_VERSION) {
		...
		pack16(msg->socket_cnt, buffer);
		for (i = 0; i < msg->socket_cnt; i++)
			power_knob_current_pack(&msg->power_info[i],
						buffer);
	} else {
		power_current_data_t *power_info = NULL;
		packstr(msg->node_name, buffer);
		if (msg->socket_cnt)
			power_info = &msg->power_info[0];
		power_knob_current_pack(power_info, buffer);
	}

}
static int
_unpack_power_knob_get_info_node_resp_msg(power_knob_get_info_node_resp_msg_t **msg,
				  Buf buffer, uint16_t protocol_version)
{
	unsigned int i;
	power_knob_get_info_node_resp_msg_t *node_data_ptr;
	uint32_t uint32_tmp;
	power_current_data_t *p;
	/* alloc memory for structure */
	xassert(msg != NULL);
	node_data_ptr = xmalloc(sizeof(power_knob_get_info_node_resp_msg_t));
	*msg = node_data_ptr;
	if (protocol_version >= SLURM_15_08_PROTOCOL_VERSION) {
		safe_unpackstr_xmalloc(&node_data_ptr->node_name,
				       &uint32_tmp, buffer);
		safe_unpack16(&node_data_ptr->socket_cnt, buffer);
		node_data_ptr->power_info = xmalloc(sizeof(power_current_data_t)
						* node_data_ptr->socket_cnt);
		for (i = 0; i < node_data_ptr->socket_cnt; ++i) {
			p = &node_data_ptr->power_info[i];
			if (power_knob_current_unpack(
				    p, buffer, 0)
			    != SLURM_SUCCESS)
				goto unpack_error;
		}
		...
		safe_unpackstr_xmalloc(&node_data_ptr->node_name,
				       &uint32_tmp, buffer);
		if (power_knob_current_unpack(node_data_ptr->power_info, buffer, 1)
		    != SLURM_SUCCESS)
			goto unpack_error;
	}
	return SLURM_SUCCESS;
...
unpack_error:
	slurm_free_power_knob_get_info_node_resp_msg(node_data_ptr);
	*msg = NULL;
	return SLURM_ERROR;
}
static void 
_pack_power_knob_get_cache_info_req_msg(power_knob_get_cache_info_req_msg_t *msg,
			Buf buffer,
			uint16_t protocol_version)
{
	xassert(msg != NULL);
...
static int 
_unpack_power_knob_get_cache_info_req_msg(power_knob_get_cache_info_req_msg_t **msg,
			Buf buffer,
			uint16_t protocol_version)
{
	power_knob_get_cache_info_req_msg_t *msg_ptr;

	xassert(msg!=NULL);

	msg_ptr = xmalloc(sizeof(power_knob_get_cache_info_req_msg_t));
	*msg = msg_ptr;
...
unpack_error:
	slurm_free_power_knob_get_cache_info_req_msg(msg_ptr);
	*msg=NULL;
	return SLURM_ERROR;
...
// TODO
static void
_pack_power_knob_get_cache_info_node_resp_msg(power_knob_get_cache_info_node_resp_msg_t *msg,
			Buf buffer,
			uint16_t protocol_version)
{
	debug3("NO! _pack_power_knob_get_cache_info_node_resp_msg");
	unsigned int i;
	xassert(msg != NULL);

	if (protocol_version >= SLURM_15_08_PROTOCOL_VERSION) {
		packstr(msg->node_name, buffer);		
		pack16(msg->cache_socket_cnt, buffer);	
		for (i = 0; i < msg->cache_socket_cnt; i++)	
			power_knob_cache_pack(&msg->perf_info[i],
						buffer);
	} else {
		cache_ref_t *perf_info = NULL;		
		packstr(msg->node_name, buffer);
		if (msg->cache_socket_cnt)	
			perf_info = &msg->perf_info[0];
		power_knob_cache_pack(perf_info, buffer);	
	}
	
}

static int 
_unpack_power_knob_get_cache_info_node_resp_msg(power_knob_get_cache_info_node_resp_msg_t **msg,
			Buf buffer,
			uint16_t protocol_version)
{
	debug3("NO! _unpack_power_knob_get_cache_info_node_resp_msg");
	unsigned int i;
	power_knob_get_cache_info_node_resp_msg_t *node_data_ptr;
	uint32_t uint32_tmp;
	cache_ref_t *p;
	/* alloc memory for structure */
	xassert(msg != NULL);
	node_data_ptr = xmalloc(sizeof(power_knob_get_cache_info_node_resp_msg_t));
	*msg = node_data_ptr;
	if (protocol_version >= SLURM_15_08_PROTOCOL_VERSION) {
		safe_unpackstr_xmalloc(&node_data_ptr->node_name,
				       &uint32_tmp, buffer);	
		safe_unpack16(&node_data_ptr->cache_socket_cnt, buffer);
		node_data_ptr->perf_info = xmalloc(sizeof(cache_ref_t)
						* node_data_ptr->cache_socket_cnt);
		for (i = 0; i < node_data_ptr->cache_socket_cnt; ++i) {
			p = &node_data_ptr->perf_info[i];	
			if (power_knob_cache_unpack(   //line 416 of src/common/power_knob.c 
				    p, buffer, 0)	
			    != SLURM_SUCCESS)
				goto unpack_error;
		}
		...
		safe_unpackstr_xmalloc(&node_data_ptr->node_name,
				       &uint32_tmp, buffer);		
		if (power_knob_cache_unpack(node_data_ptr->perf_info, buffer, 1)
		    != SLURM_SUCCESS)	
			goto unpack_error;
	}	
	return SLURM_SUCCESS;	
...
unpack_error:
	slurm_free_power_knob_get_cache_info_node_resp_msg(node_data_ptr);	//line 1019 of rc/common/slurm_protocol_defs.c
	*msg = NULL;
	return SLURM_ERROR;
}

static void 
_pack_power_knob_set_node_resp_msg(power_knob_set_node_resp_msg_t *msg,
			Buf buffer,
			uint16_t protocol_version)
{
	return SLURM_ERROR;
...
static int 
_unpack_power_knob_set_node_resp_msg(power_knob_set_node_resp_msg_t **msg,
			Buf buffer,
			uint16_t protocol_version)
{
	return SLURM_ERROR;
			...
			uint16_t protocol_version)
{
	...
	pack32(msg->cap_info,buffer);
	pack32(msg->cap_info2,buffer);
...
{
	int i;
	power_knob_cap_req_msg_t	*msg_ptr;
	uint32_t cap_info;
	debug ("inside unpack of slurm_protocol_pack");
	xassert(msg!=NULL);

	msg_ptr = xmalloc(sizeof(power_knob_cap_req_msg_t));
	*msg = msg_ptr;	
	
	safe_unpack32(&msg_ptr->cap_info,buffer);
	safe_unpack32(&msg_ptr->cap_info2,buffer);
	
	return SLURM_SUCCESS;

unpack_error:
	 slurm_free_power_knob_cap_req_msg(msg_ptr);
	*msg=NULL;
	return SLURM_ERROR;
}
static void _pack_power_knob_cap_resp_msg(
...
{
	int i;
	power_schedule_slurmd_req_msg_t	*msg_ptr;
	uint32_t dummy;
	debug ("inside unpack of slurm_protocol_pack");
	xassert(msg!=NULL);

	msg_ptr = xmalloc(sizeof(power_schedule_slurmd_req_msg_t));
	*msg = msg_ptr;	
...
unpack_error:
	slurm_free_power_schedule_slurmd_req_msg(msg_ptr);
	*msg=NULL;
	return SLURM_ERROR;
}
