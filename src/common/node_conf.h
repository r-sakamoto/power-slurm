...
struct _cpu_socket_power
{
   float current_power_usage; /* real power usage */
   int   power_cap_setting;   /* power cap setting, 1 if it is capped, otherwise 0 */
   time_t last_cap_time;		/* time of the last power cap */
   float current_power_cap;   /* power cap value now */
   double *pmc;               /* 4 performance monitoring counters */
};

struct node_record {
	struct _cpu_socket_power *cpu_socket_power; 
	...
	power_data_t *power_info;	/* power performance data*/	
