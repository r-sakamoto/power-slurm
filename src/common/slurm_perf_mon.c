/*****************************************************************************\
 *  slurm_perf_mon.c - implementation-node performance monitor
 *  definitions
 *****************************************************************************
...
typedef struct slurm_perf_mon_ops {
...
} slurm_perf_mon_ops_t;
/*
 * These strings must be kept in the same order as the fields
 * declared for slurm_perf_mon_ops_t.
 */
static const char *syms[] = {
	"perf_mon_p_update_node_energy",
	"perf_mon_p_get_data",
	"perf_mon_p_set_data",
	"perf_mon_p_conf_options",
	"perf_mon_p_conf_set",
	"perf_mon_p_conf_values",
};

static slurm_perf_mon_ops_t ops;
...
extern int slurm_perf_mon_init(void)
{
	int retval = SLURM_SUCCESS;
	char *plugin_type = "perf_mon";
	char *type = NULL;
...
extern int perf_mon_startpoll(uint32_t frequency)
{
	...
	if (frequency == 0) {   /* don't want dynamic monitoring? */
		debug2("perf_mon dynamic logging disabled");
		return retval;
	}
	...
	if (pthread_create(&_watch_node_thread_id, &attr, &_watch_node, NULL)) {
		debug("perf_mon failed to create _watch_node "
		      "thread: %m");
	} else
		debug3("perf_mon dynamic logging enabled");
