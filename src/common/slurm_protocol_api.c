...
char *slurm_get_plugin_dir(void)
{
	...
	slurm_ctl_conf_t *conf;
    //printf("CAO: slurm_get_plugin_dir\n");
	if (slurmdbd_conf) {
		plugin_dir = xstrdup(slurmdbd_conf->plugindir);
        //printf("CAO: slurm_get_plugin_dir: plugin_dir = xstrdup(slurmdbd_conf->plugindir) = %s\n", plugin_dir);
	} else {
		conf = slurm_conf_lock();
		plugin_dir = xstrdup(conf->plugindir);
		slurm_conf_unlock();
        //printf("CAO: slurm_get_plugin_dir: plugin_dir = xstrdup(conf->plugindir); = %s\n", plugin_dir);
	}
    debug3("CAO slurm_get_plugin_dir: plugin_dir = '%s'\n", plugin_dir);
	return plugin_dir;
}
...
/* slurm_get_power_knob_type
 * get PowerKnobType from slurmctld_conf object
 * RET char *   - power_knob type, MUST be xfreed by caller
 */
char *slurm_get_power_knob_type(void)
{
	char *power_knob_type = NULL;
	slurm_ctl_conf_t *conf;
	...
	if (slurmdbd_conf) {
		...
		conf = slurm_conf_lock();
		power_knob_type =
			xstrdup(conf->power_knob_type);
		slurm_conf_unlock();
	}
	return power_knob_type;
}
...
/* slurm_get_select_params
 * RET char * - Value of SelectParameters, MUST be xfreed by caller */
extern char *slurm_get_select_params(void)
{
	char *params = 0;
	slurm_ctl_conf_t *conf;
 	...
 	if (slurmdbd_conf) {
		...
		conf = slurm_conf_lock();
		params = xstrdup(conf->select_params);
		slurm_conf_unlock();
	}
	return params;
}

/* slurm_get_powerallocator_type
 * get  type from slurmctld_conf object
 * RET char *   - power allocator type, MUST be xfreed by caller 
 */
char *slurm_get_powerallocator_type(void)
{
	char *powerallocator_type = NULL;
	slurm_ctl_conf_t *conf;

	if (slurmdbd_conf) {
	} else {
		conf = slurm_conf_lock();
		powerallocator_type = xstrdup(conf->power_allocatortype);
		slurm_conf_unlock();
	}
	return powerallocator_type;
}

/* slurm_get_poweranalyzer_type
 * get sched type from slurmctld_conf object
 * RET char *   - poweranalyzer type, MUST be xfreed by caller
 */
char *slurm_get_poweranalyzer_type(void)
{
	char *poweranalyzer_type = NULL;
	slurm_ctl_conf_t *conf;

	if (slurmdbd_conf) {
	} else {
		conf = slurm_conf_lock();
		poweranalyzer_type = xstrdup(conf->power_analyzertype);
		slurm_conf_unlock();
	}
	return poweranalyzer_type;
}


/* slurm_get_powerascheduleslurmd_type
 * get  type from slurmctld_conf object
 * RET char *   - power allocator type, MUST be xfreed by caller 
 */
char *slurm_get_powerascheduleslurmd_type(void)
{
	char *powerascheduleslurmd_type = NULL;
	slurm_ctl_conf_t *conf;

	if (slurmdbd_conf) {
	} else {
		conf = slurm_conf_lock();
		powerascheduleslurmd_type = xstrdup(conf->power_schedule_slurmd);
		slurm_conf_unlock();
	}
	return powerascheduleslurmd_type;
}
...
{
    debug3("CAO: int slurm_receive_msg(int fd, slurm_msg_t *msg, int timeout)");
	char *buf = NULL;
	size_t buflen = 0;
	int rc;
	Buf buffer;
	...
	msg->conn_fd = fd;
	debug2("Linh: inside slurm_receive_msg1 ");
	if (timeout <= 0)
	 ...
	 *  the message.
	 */
	 debug2("Linh: inside slurm_receive_msg2 ");
	if (slurm_msg_recvfrom_timeout(fd, &buf, &buflen, 0, timeout) < 0) {
		rc = errno;
		goto endit;
	}
	debug2("Linh: inside slurm_receive_msg3 ");
#if	_DEBUG
...
int slurm_send_node_msg(int fd, slurm_msg_t * msg)
{
    ...
    debug3("CAO int slurm_send_node_msg()");
	if (msg->conn) {
		persist_msg_t persist_msg;
        ...
        debug3("CAO int slurm_send_node_msg(): return rc on if (msg->conn)");
		return rc;
	}
	...
	free_buf(buffer);
    debug3("CAO int slurm_send_node_msg(): return rc at the end");
	return rc;
}
			       ...
			       slurm_msg_t *resp, int timeout)
{
    ...
    debug3("CAO slurm_send_recv_msg(): 1. slurm_send_node_msg()");
	if (slurm_send_node_msg(fd, req) >= 0) {
        ...
        debug3("CAO slurm_send_recv_msg(): 2. slurm_receive_msg()");
		rc = slurm_receive_msg(fd, resp, timeout);
	}
    else 
        debug3("CAO slurm_send_recv_msg(): 1. failed do to slurm_send_node_msg()");
...
int slurm_send_recv_node_msg(slurm_msg_t *req, slurm_msg_t *resp, int timeout)
{
	...
	if ((fd = slurm_open_msg_conn(&req->address)) < 0)
		return -1;
	debug2("Linh: inside slurm_send_recv_node_msg");
