...
s_p_options_t slurm_conf_options[] = {
	...
	{"SelectParameters", S_P_STRING},
	{"PowerAllocatorType", S_P_STRING},

	{"PowerAllocatorInterval", S_P_UINT32},
	{"PowerAnalyzerType", S_P_STRING},	
	{"PowerAnalyzerInterval", S_P_UINT32},	
	{"PowerMonitorInterval", S_P_UINT32},	
	{"PowerAlert", S_P_UINT32},
	...
	{"PowerBudget", S_P_UINT32}, 
	
	{"PowerScheduleSlurmdType", S_P_STRING}, 
	{NULL}
};
...
free_slurm_conf (slurm_ctl_conf_t *ctl_conf_ptr, bool purge_node_hash)
{
	...
	xfree (ctl_conf_ptr->power_knob_type);	
	...
	xfree (ctl_conf_ptr->select_params);
	xfree (ctl_conf_ptr->power_allocatortype);
	xfree (ctl_conf_ptr->power_analyzertype);	
	xfree (ctl_conf_ptr->power_schedule_slurmd);
...
init_slurm_conf (slurm_ctl_conf_t *ctl_conf_ptr)
{
	...
	ctl_conf_ptr->z_32 = (uint32_t) NO_VAL;
	
	ctl_conf_ptr->power_allocatorinterval = (uint32_t) NO_VAL;
	
	ctl_conf_ptr->power_analyzerinterval = (uint32_t) NO_VAL;
	
	ctl_conf_ptr->power_monitorinterval = (uint32_t) NO_VAL;
	ctl_conf_ptr->power_alert = (uint32_t) NO_VAL;
	_free_name_hashtbl();
	_init_name_hashtbl();
...
_validate_and_set_defaults(slurm_ctl_conf_t *conf, s_p_hashtbl_t *hashtbl)
{
	...
	if (!s_p_get_string(&conf->power_knob_type,
			    "PowerKnobType", hashtbl))
		conf->power_knob_type =
			xstrdup(DEFAULT_POWER_KNOB_TYPE);
	...
	s_p_get_string(&conf->select_params, "SelectParameters", hashtbl);
	...
	if (!s_p_get_string(&conf->power_allocatortype, "PowerAllocatorType", hashtbl))
	...
	if (!s_p_get_string(&conf->power_schedule_slurmd, "PowerScheduleSlurmdType", hashtbl))
		conf->power_schedule_slurmd = xstrdup(DEFAULT_POWER_SCHEDULE_SLURMD_TYPE);
	...
	if (!s_p_get_uint32(&conf->z_32, "PowerBudget", hashtbl))
		conf->z_32 = DEFAULT_POWER_BUDGET;
