			...
			struct config_record *config_ptr, char *node_name)
{
	...
	/* allocate memory for cpu_socket_power*/
	node_ptr->cpu_socket_power = (struct _cpu_socket_power *) xmalloc(config_ptr->sockets*sizeof(struct _cpu_socket_power));
   
	/* allocate memory for 4 performance monitoring counter */
	int i;
	for (i = 0; i < config_ptr->sockets; i++){
		node_ptr->cpu_socket_power->pmc = xmalloc(4*sizeof(double));
	}
	/* allocate memory for power knob */
	node_ptr->power_info = power_knob_data_alloc();
	node_ptr->power_info->power_cap = power_knob_cap_alloc(config_ptr->sockets);
	node_ptr->power_info->current_power = power_knob_current_alloc(config_ptr->sockets);
	node_ptr->power_info->cache_reference = power_knob_cache_alloc(config_ptr->sockets);
...
extern void purge_node_rec (struct node_record *node_ptr)
{
	...
	/* free memory for cpu_socket_power*/
	int i;
	for (i = 0; i < node_ptr->sockets; i++){
		xfree(node_ptr->cpu_socket_power->pmc);
	}
	xfree(node_ptr->cpu_socket_power);
	
	power_knob_cap_destroy(node_ptr->power_info->power_cap);
	power_knob_current_destroy(node_ptr->power_info->current_power);
	power_knob_cache_alloc(node_ptr->power_info->cache_reference);

	xfree(node_ptr->power_info);	
}
