...
static int _rpc_power_knob_get_info(slurm_msg_t *msg);
static int _rpc_power_knob_get_cache_info(slurm_msg_t *msg);
static int _rpc_power_knob_set(slurm_msg_t *msg);
static int _rpc_power_knob_cap(slurm_msg_t *msg);

static int _rpc_power_schedule_slurmd(slurm_msg_t *msg);

static int do_power_schedule_slurmd();
...
{
    debug3("CAO: do slurmd_req(slurm_msg_t *msg)");
	int rc;
    ...
    debug3("CAO: do slurmd_req():switch (msg->msg_type)=%i", msg->msg_type);
	switch (msg->msg_type) {
...
static int
_rpc_power_knob_get_cache_info(slurm_msg_t *msg)
{
	debug("rpc_power_knob_get_cache_info running");
	int        rc = SLURM_SUCCESS;
	uid_t req_uid = g_slurm_auth_get_uid(msg->auth_cred,
					     slurm_get_auth_info());
	int i;
	...
	if (!_slurm_authorized_user(req_uid)) {
		error("Security violation, power_knob_get_info RPC from uid %d",
		      req_uid);
		if (first_msg) {
			error("Do you have SlurmUser configured as uid %d?",
			      req_uid);
	...
	if (rc != SLURM_SUCCESS) {
		if (slurm_send_rc_msg(msg, rc) < 0)
			error("Error responding to power knob get info: %m");
	} else {
		slurm_msg_t resp_msg;
		power_knob_get_cache_info_node_resp_msg_t cache_msg; //Define at line 2237 of slurm/slurm.h
		uint16_t cache_socket_cnt;
		power_knob_get_cache_info_req_msg_t *req = msg->data; //Define at line 2233 of slurm/slurm.h

		//power_knob_g_get_data(POWER_KNOB_DATA_SOCKET_CNT,&socket_cnt);
		power_knob_g_get_cache_data(CACHE_POWER_KNOB_DATA_SOCKET_CNT,&cache_socket_cnt);   //Define at line 451 of src/common/power_knob.c
		
		memset(&cache_msg, 0, sizeof(power_knob_get_cache_info_node_resp_msg_t));

		//power_msg.socket_cnt = socket_cnt;
		//power_msg.power_info = power_knob_current_alloc(socket_cnt);

		cache_msg.cache_socket_cnt = cache_socket_cnt;
		cache_msg.perf_info = power_knob_cache_alloc(cache_socket_cnt);

		//power_knob_g_get_data(POWER_KNOB_DATA_NODE_POWER,power_msg.perf_info);
		power_knob_g_get_cache_data(CACHE_POWER_KNOB_DATA_NODE_POWER, cache_msg.perf_info);
		
		slurm_msg_t_copy(&resp_msg, msg);
		resp_msg.msg_type = RESPONSE_POWER_KNOB_GET_CACHE_INFO;
		resp_msg.data     = &cache_msg;

		slurm_send_node_msg(msg->conn_fd, &resp_msg);
		
		//power_knob_current_destroy(power_msg.power_info);
		power_knob_cache_destroy(cache_msg.perf_info); //Define at line 287 of src/common/power_knob.c
...
static int
_rpc_power_knob_set(slurm_msg_t *msg)
{
/*
	int        rc = SLURM_SUCCESS;
	uid_t req_uid = g_slurm_auth_get_uid(msg->auth_cred,
	...
	if (rc != SLURM_SUCCESS) {
		...
		power_knob_set_req_msg_t *req = msg->data;

		power_knob_g_set_data(req);
		
	}
	return rc;
*/
	debug("LINH rpc_power_knob_seet_info running");
	int        rc = SLURM_SUCCESS;
	uid_t req_uid = g_slurm_auth_get_uid(msg->auth_cred,
					     slurm_get_auth_info());
	int i;
	...
	if (rc != SLURM_SUCCESS) {
		...
		slurm_msg_t resp_msg;
		power_knob_get_info_node_resp_msg_t power_msg;
		uint16_t socket_cnt;
		power_knob_set_req_msg_t *req = msg->data;
		socket_cnt = 1;

/*		debug3("    _rpc_power_knob_set checking socket_cnt");
		power_knob_g_get_data(POWER_KNOB_DATA_SOCKET_CNT,&socket_cnt);
		debug3("    _rpc_power_knob_set socket_cnt %d",socket_cnt);

		if(socket_cnt != req->socket_cnt){
			error("Error cap socket val is not given value");
		}
*/
		
		debug3("    _rpc_power_knob_set setting the cap");
		power_knob_g_set_data(req->cap_info);
		debug3(" Receive set data is %d",req->cap_info);
//		power_knob_g_set_data(req);
		
		debug3("    _rpc_power_knob_set end setting the cap");

		memset(&power_msg, 0, sizeof(power_knob_get_info_node_resp_msg_t));

		power_msg.socket_cnt = socket_cnt;
		power_msg.power_info = power_knob_current_alloc(socket_cnt);

		debug3("    _rpc_power_knob_set getting the cap");
		power_knob_g_get_data(POWER_KNOB_DATA_NODE_POWER,power_msg.power_info);
		debug3("    _rpc_power_knob_set geted the cap");

		slurm_msg_t_copy(&resp_msg, msg);
		resp_msg.msg_type = RESPONSE_POWER_KNOB_GET_INFO;
		resp_msg.data     = &power_msg;

		debug3("    _rpc_power_knob_set sending");
		slurm_send_node_msg(msg->conn_fd, &resp_msg);
		debug3("    _rpc_power_knob_set done sending");
...
static int do_power_schedule_slurmd()
{
	debug ("INSIDE SCHEDULE SLURMD PLUGIN POWER CAP");
}
static int
_rpc_power_knob_cap(slurm_msg_t *msg)
{

	debug("LINH _rpc_power_knob_cap running");
	int        rc = SLURM_SUCCESS;
	uid_t req_uid = g_slurm_auth_get_uid(msg->auth_cred,
					     slurm_get_auth_info());
	int i;
	...
	if (rc != SLURM_SUCCESS) {
		...
		power_knob_get_info_node_resp_msg_t power_msg;
		uint16_t socket_cnt;
		power_knob_cap_req_msg_t *req = msg->data;
		socket_cnt = 2;

		
		debug3("    _rpc_power_knob_cap setting the cap");
	//	power_knob_g_set_data(req->cap_info);
		power_knob_g_set_data(req);
		
		debug3(" Receive set data 1 is %d",req->cap_info);
		
		debug3(" Receive set data 2 is %d",req->cap_info2);
		
		debug3("    _rpc_power_knob_cap end setting the cap");
		...
		debug3("    _rpc_power_knob_cap getting the cap");
		power_knob_g_get_data(POWER_KNOB_DATA_NODE_POWER,power_msg.power_info);
		debug3("    _rpc_power_knob_cap geted the cap");

		slurm_msg_t_copy(&resp_msg, msg);
		resp_msg.msg_type = RESPONSE_POWER_KNOB_GET_INFO;
		resp_msg.data     = &power_msg;

		debug3("    _rpc_power_knob_cap sending");
		slurm_send_node_msg(msg->conn_fd, &resp_msg);
		debug3("    _rpc_power_knob_cap done sending");
...
static int
_rpc_power_schedule_slurmd(slurm_msg_t *msg)
{

	debug("LINH _rpc_power_schedule slurmd running");
	int        rc = SLURM_SUCCESS;
	uid_t req_uid = g_slurm_auth_get_uid(msg->auth_cred,
					     slurm_get_auth_info());
	int i;
	...
	if (rc != SLURM_SUCCESS) {
		...
		power_knob_get_info_node_resp_msg_t power_msg;
		uint16_t socket_cnt;
		power_schedule_slurmd_req_msg_t *req = msg->data;
		socket_cnt = 2;

	    do_power_schedule_slurmd();

		
		debug3("    _rpc_power_schedule_slurmd setting the cap");
	//	power_knob_g_set_data(req->cap_info);

//NOTE  	power_knob_g_set_data(req);
...
static int
_rpc_power_knob_get_info(slurm_msg_t *msg)
{
	debug("rpc_power_knob_get_info running");
	int        rc = SLURM_SUCCESS;
	uid_t req_uid = g_slurm_auth_get_uid(msg->auth_cred,
					     slurm_get_auth_info());
	int i;
	...
	if (rc != SLURM_SUCCESS) {
		...
		power_knob_get_info_node_resp_msg_t power_msg;
		uint16_t socket_cnt;
		power_knob_get_info_req_msg_t *req = msg->data;
