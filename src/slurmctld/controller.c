...
int main(int argc, char **argv)
{
	...
	debug ("Power budget is %d", slurmctld_conf.z_32);
	...
	while (1) {
		...
		if (!slurmctld_primary) {
			...
			power_allocator_fini();	/* make sure shutdown */
			power_analyzer_fini();	/* make sure shutdown */
			power_schedule_slurmd_fini();
		...
		if (power_allocator_init() != SLURM_SUCCESS )
			fatal( "failed to initialize power allocator plugin");
		if (power_analyzer_init() != SLURM_SUCCESS )
			fatal( "failed to initialize power analyzer plugin");
		if (power_schedule_slurmd_init() != SLURM_SUCCESS )
			fatal( "failed to initialize power schedule slurmd plugin");
	
//		power_analyzer_g_analyze_interval();
		/*
		 * create attached thread for node power monitor
  		 */
		start_power_monitor(&slurmctld_config.thread_id_power_monitor);		
