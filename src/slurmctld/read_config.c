...
int read_slurm_conf(int recover, bool reconfig)
{
	...
	if (power_allocator_init() != SLURM_SUCCESS)
		fatal("Failed to initialize powerallocator plugin");

	if (power_analyzer_init() != SLURM_SUCCESS)
		fatal("Failed to initialize poweranalyzer plugin");

	if (power_schedule_slurmd_init() != SLURM_SUCCESS)
		fatal("Failed to initialize power_schedule_slurmd plugin");
	...
	power_allocator_g_reconfig();
	power_analyzer_g_reconfig();
	power_schedule_slurmd_g_reconfig();
