...
typedef struct slurmctld_config {
	...
	pthread_t thread_id_power_monitor;
 ...
 */
struct job_record {
	float perf_requirement; /* performance requirement, forexample 0.9 */
	int   compensate_power_cap_counter; /* counter for times when power
       	                              * cap is lower than neccessary */
